...
#         #new_state.lin_vel = self.cf_state.forces / self.cf_physical_params.DRONE_MASS - np.dot(np.linalg.inv(rotation_matrix),np.array([0, 0, self.cf_physical_params.G])) - np.cross(self.cf_state.ang_vel, self.cf_state.lin_vel)
...
#         #print(np.dot(rotation_matrix,self.cf_state.forces))
...
#         self.msg_odom.pose.pose.orientation.x = self.cf_state.attitude[0]
#         self.msg_odom.pose.pose.orientation.y = self.cf_state.attitude[1]
#         self.msg_odom.pose.pose.orientation.z = self.cf_state.attitude[2]
