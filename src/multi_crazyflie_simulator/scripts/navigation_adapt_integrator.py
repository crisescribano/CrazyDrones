...
def navigation():
  ...
  global reference_pose
  global agent_pose 
  global other_agents_pose_1  #agent 2 for 1, agent 1 for 2, agent 1 for 3, agent 1 for 4, agent 1 for 5, agent 1 for 6
  global other_agents_pose_2  #agent 3 for 1, agent 3 for 2, agent 2 for 3, agent 2 for 4, agent 2 for 5, agent 2 for 6
  global other_agents_pose_3  #agent 4 for 1, agent 4 for 2, agent 4 for 3, agent 3 for 4, agent 3 for 5, agent 3 for 6
  global other_agents_pose_4  #agent 5 for 1, agent 5 for 2, agent 5 for 3, agent 5 for 4, agent 4 for 5, agent 4 for 6
  global other_agents_pose_5  #agent 6 for 1, agent 6 for 2, agent 6 for 3, agent 6 for 4, agent 6 for 5, agent 5 for 6
  global PoI
  global other_agent_1_arrived
  ...
  PoI = np.array([[0, 0, 5], [4, 5, 3],[-2, 4, 2],[3, -2, 3]])

  #total of 5 agents

  force_pub = rospy.Publisher('uav_force_reference', mav_msgs.msg.TorqueThrust, queue_size = 100)

  theta_hat_pub = rospy.Publisher('theta_hat_pub', std_msgs.msg.Float64, queue_size = 100)
  d_hat_pub = rospy.Publisher('d_hat_pub', std_msgs.msg.Float64, queue_size = 100)
  f_hat_pub = rospy.Publisher('f_hat_pub', std_msgs.msg.Float64, queue_size = 100)
  ...
  rospy.Subscriber('other_uav_odometry_4', nav_msgs.msg.Odometry,callback_sub_othet_uav_odometry_4)
  rospy.Subscriber('other_uav_odometry_5', nav_msgs.msg.Odometry,callback_sub_othet_uav_odometry_5)
  ...
  if agent_number == 1:
   	e_p_pub = rospy.Publisher('leader_error', std_msgs.msg.Float64, queue_size = 100)
   	leader_time_pub = rospy.Publisher('leader_time', std_msgs.msg.Float64, queue_size = 100)
  ...
  ki = 5
  #if agent_number==5 or agent_number==6:
  #  ki = 10
  ...
  other_agents_pose_4 = nav_msgs.msg.Odometry()
  other_agents_pose_5 = nav_msgs.msg.Odometry()
  #rospy.sleep(2)
  ...
  while not rospy.is_shutdown():
    ...
    # error_z = x[2] - 3
    # error_d_z = v[2]
    # force_z = 9.81*1.56 - pid_p*error_z - pid_d*error_d_z
    ...
    x_other_4, v_other_4 = position_and_velocity_from_odometry(other_agents_pose_4)
    x_other_5, v_other_5 = position_and_velocity_from_odometry(other_agents_pose_5)
...
F
    ...
    if agent_number==1 or agent_number==3:
      if eta_con_3 < 0:
        beta_con_3 = 0
        grad_beta_con_3 = np.zeros(3)
        grad_beta_con_dot_3 = np.zeros(3)
    ...
    #print 'agent = ', agent_number, ', control = ', np.array([control[0],control[1],control[2]])
    ...
    # if agent_number==1:
    # 	beta_12_col_msg = std_msgs.msg.Float64(); beta_12_col_msg.data = np.linalg.norm(beta_col_1)
    # 	beta_13_col_msg = std_msgs.msg.Float64(); beta_13_col_msg.data = np.linalg.norm(beta_col_2)
    # 	beta_14_col_msg = std_msgs.msg.Float64(); beta_14_col_msg.data = np.linalg.norm(beta_col_1)
    ...
    theta_hat_msg.data = np.linalg.norm(theta_hat)
    theta_hat_pub.publish(theta_hat_msg)

    d_hat_msg = std_msgs.msg.Float64()
    d_hat_msg.data = d_b_hat
    d_hat_pub.publish(d_hat_msg)

    f_hat_msg = std_msgs.msg.Float64()
    f_hat_msg.data = f_b_hat
