...
class CF_model {
	public:
		CF_model(ros::NodeHandle* nodehandle, std::string topic_, std::string agent_number_);
	...
	private:
		...
		ros::Publisher  pub_pos_odom;

		geometry_msgs::PoseStamped msgPose; 
		nav_msgs::Odometry msgPoseOdom; 
...
CF_model::CF_model(ros::NodeHandle* nodehandle, std::string topic_, std::string agent_number_):nh_(*nodehandle){	
	
    topic = topic_;
    agent_number = agent_number_;
    ...
    pub_pos_odom = nh_.advertise<nav_msgs::Odometry>(topic + "/out_pos_odometry", 1000);
}
...
void CF_model::publishPose(){
    ...
    msgPoseOdom.twist.twist.linear.x = cf_state.lin_vel.x();
    msgPoseOdom.twist.twist.linear.y = cf_state.lin_vel.y();
    msgPoseOdom.twist.twist.linear.z = cf_state.lin_vel.z();
    msgPoseOdom.pose.pose.orientation.x = quaternion[0];
    msgPoseOdom.pose.pose.orientation.y = quaternion[1];
    msgPoseOdom.pose.pose.orientation.z = quaternion[2];
    msgPoseOdom.pose.pose.orientation.w = quaternion[3];
    pub_pos_odom.publish(msgPoseOdom);
