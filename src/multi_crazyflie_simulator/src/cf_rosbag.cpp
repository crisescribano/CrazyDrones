...
void receivedPose(const nav_msgs::Odometry::ConstPtr& msg){
    ...
    bag.write(msg->header.frame_id, ros::Time::now(), msg);
}
...
int main(int argc, char **argv)
{
  ...
  bag.open("./../Crazydrones/test_" + currentTime + ".bag", rosbag::bagmode::Write);
  ...
  for(int i = 0 ; i < numQuadcopter ; i++){
  	ros::Subscriber sub = n.subscribe("/crazyflie_" + std::to_string(i) + "/out_pos", 1000, receivedPose);
  }
